...
usage() {
    echo "Usage: $0 <fastq_directory> <log_file> <database_list_file> [kmer_size] [output_suffix]"
    ...
    echo "  kmer_size:          K-mer size for straingst kmerize (default: 23)"
...
echo "Using kmer size: $kmer_size"
echo "Using output suffix: $output_suffix"
...
# Check if straingst is available
if ! command -v straingst &> /dev/null; then
    echo "Error: 'straingst' command not found. Please ensure StrainGE is installed and in your PATH."
    exit 1
fi

# Log the processing parameters
echo "StrainGE Processing Parameters:" >> "$log_file"
echo "K-mer size: $kmer_size" >> "$log_file"
...
while IFS= read -r -d '' r1_file; do
    ...
    hdf5_file="${output_prefix}_${output_suffix}.hdf5"
    
    # Build StrainGE commands
    strainscan_cmd="straingst kmerize -k $kmer_size -o '$hdf5_file' '$r1_file' '$r2_file'; "
    
    for db in "${databases[@]}"; do
        # Replace spaces and special characters in output file name
        safe_db=$(echo "$db" | tr ' ' '_' | tr '.' '_')
        output_tsv="${output_prefix}_${output_suffix}_${safe_db}.tsv"
        database_file="${db}_pan-genome-db.hdf5"
        
        # Check if database file exists
        if [ ! -f "$database_file" ]; then
            echo "Warning: Database file '$database_file' not found, skipping..." | tee -a "$log_file"
            continue
        fi
        
        strainscan_cmd+="straingst run -o '$output_tsv' '$database_file' '$hdf5_file'; "
    done
    
    # Measure time and memory usage
    echo "Starting StrainGE processing..." >> "$log_file"
